...
LPSTR removeHtmlTags(const LPSTR html)
{
	int len = std::strlen(html);
	LPSTR sortie = new char[len + 1];
...
void get_All_Text(std::string& entreeURL)
{
						...
						if (dwSize > 0) {
							...
							if (!pszSortieTempon) {
								...
								if (WinHttpReadData(hRequete, (LPVOID)pszSortieTempon,
									...
									std::cout << removeHtmlTags(pszSortieTempon);
								}
